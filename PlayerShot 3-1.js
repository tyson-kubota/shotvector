...
function Start () {
	...
	// MyLayerMask = (1 << LayerMask.NameToLayer("PlayerLayer")) | (1 << LayerMask.NameToLayer("PlayerProjectileLayer"))
	//  | (1 << LayerMask.NameToLayer("EnemyProjectileLayer")) | (1 << LayerMask.NameToLayer("DestroyedEnemy"))
	//  | (1 << LayerMask.NameToLayer("ExplosionCheck")) | (1 << LayerMask.NameToLayer("Ignore Raycast"))
	//  | (1 << LayerMask.NameToLayer("reticle")) | (1 << LayerMask.NameToLayer("reticle-chevrons"));

	//MyLayerMask = (1 << LayerMask.NameToLayer("landscape")) | (1 << LayerMask.NameToLayer("EnemyLayer")) | (1 << LayerMask.NameToLayer("sky"));
	...
	if (Time.timeScale < 1.0) {fireRate = (origFireRate * 2 * Time.timeScale);}
		...
		for (var i = 0; i < Input.touchCount; ++i) {
			if (Input.GetTouch(i).phase == TouchPhase.Began || Input.GetTouch(i).phase == TouchPhase.Moved || Input.GetTouch(i).phase == TouchPhase.Stationary) {
				...
				var ray = mainCamera.ScreenPointToRay (Vector3(touchPos.x, touchPos.y, 0));
				var hit : RaycastHit;
				//Debug.DrawLine (ray.origin, hit.point, Color.red, 10, false);
				...
				//pos = Vector3.zero;
				var woo : Vector3 = Vector3(-9000,-1111,-1111);
				pos = woo;
				...
				if (Physics.Raycast (ray, hit, 5000, MyLayerMask)) {
					...
					// rotation here seems weird. shortest path ends up pointing backwards?
